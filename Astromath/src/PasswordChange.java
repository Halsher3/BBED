...
public class PasswordChange extends JPanel {
	...
	/**
	 * Creates the PasswordChange page, here the student is able to change their
	 * password and the application will store that in the database. The password
	 * must follow the restrictions listed, and there are checks for that.
	 */
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (checkString(temp) && temp.length() >= 6) {
					...
					try {
						...
						String query = "UPDATE userinfo SET password = '" + newPassword + "' WHERE userID = '"
								+ student.getAccNum() + "'";
