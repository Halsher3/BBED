...
public class Login extends JPanel {
	...
	/**
	 * Creates the Login page, this is the first page that the user will be able to
	 * see launching the application. Here they will be able to login with their
	 * username and password, or they can be sent to the AccountCreations page. Once
	 * the username and password is submitted, the application will check with the
	 * database and see if that account exists and the information is correct. If it
	 * isn't a piece of text will say its incorrect, if it is then a popup will say
	 * that the login was successful.
	 * 
	 * Information about switch_screen() is included below.
	 */
	public Login(JLayeredPane lp, Test test, Student student, Connection con) {
		...
		btnLogin.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					var sql = "Select * from userinfo where username='" + textUsername.getText() + "' and password='"
					...
					if (rs.next()) {
						...
						String query = "Select userID, userLevel, grade, email, name from userinfo where username = '"
		...
		/*
		 * OLD LOGIN STUFF btnLogin.addMouseListener(new MouseAdapter() {
		 * 
		 * @Override public void mouseClicked(MouseEvent e) {
		 * if(goku.getLogin(textUsername.getText(),
		 * String.valueOf(textPassword.getPassword()))) { MainWindow panel_home = new
		 * MainWindow(lp); switch_screen(panel_home.getPanel(), lp); } else {
		 * 
		 * labelIncorrect.setVisible(true);; } } });
		 */
	...
	/*
	 * This is the switch_screen function, which will appear in EVERY GUI class from here on out, with potentially different parameters.
	 * Here the current panel(s) that is being displayed will be erased, and seamlessly the elements of the next JPanel will be 
	 * displayed and the code for the next page will be ran.
	 * The parameters are the same as the constructors.
	 * 
	 * It passes a layered pane object in order to make sure the one from Visual, updates with the current page.
	 */
