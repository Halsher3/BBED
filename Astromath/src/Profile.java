...
public class Profile extends JPanel {
	...
	/**
	 * This creates the Profile page. Here the student can view their current profile information 
	 * which is grabbed from the database.
	 * Some of these include their profile picture, name, state, about me, level.
	 * They can also be redirected to the ProfileSettings page by clicking the test next to the pencil.
	 */
	public Profile(JLayeredPane lp, Test test, Student student, Connection con) {
		...
		try {
			...
			if (rs.next()) {
				...
				quer = "Select userProfilePic,userAboutMe,profileState from profileinfo where userID = '"
						+ student.getAccNum() + "'";
				...
				quer = "INSERT INTO profileinfo(`userID`, `userProfilePic`, `userAboutMe`, `profileState`) "
						...
						+ "VALUES ('" + student.getAccNum() + "','" + defaultPFP + "','" + profileAboutMe + "','"
						+ profileState + "')";
