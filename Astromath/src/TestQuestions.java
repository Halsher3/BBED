			...
			public void mouseClicked(MouseEvent e) {
				...
				try {
					...
					if (tempTest.equals("Addition Test") || tempTest.equals("Multiplication Test")) {
						String query = "Update testinfo set testScore1 = '" + grades + "' where userID = '"
								+ student.getAccNum() + "'";
						Statement st = con.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,
								ResultSet.CONCUR_READ_ONLY);
						st.executeUpdate(query);
					} else if (tempTest.equals("Subtraction Test") || tempTest.equals("Division Test")) {
						String query = "Update testinfo set testScore2 = '" + grades + "' where userID = '"
								+ student.getAccNum() + "'";
						Statement st = con.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,
								ResultSet.CONCUR_READ_ONLY);
						st.executeUpdate(query);
						...
						String query = "Update testinfo set testScore3 = '" + grades + "' where userID = '"
								+ student.getAccNum() + "'";
						Statement st = con.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,
								ResultSet.CONCUR_READ_ONLY);
						st.executeUpdate(query);
					} else if (tempTest.equals("Fill In The Blank Test")) {
						String query = "Update testinfo set testScore4 = '" + grades + "' where userID = '"
								+ student.getAccNum() + "'";
						Statement st = con.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,
								ResultSet.CONCUR_READ_ONLY);
						st.executeUpdate(query);
				...
				mpAnswer1.addMouseListener(new MouseAdapter() {
					...
					public void mouseClicked(MouseEvent e) {
						if (answerKey[0] == equation[3]) {
							...
							TestQuestions panel_question = new TestQuestions(lp, test, questionSelect, operand, student,
									con);
